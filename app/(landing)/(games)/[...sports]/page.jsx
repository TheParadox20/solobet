...
export default function Page({params}) {
  ...
  let { data, error, isLoading, mutate } = useSWR(['/games',{path:path.join('/'),search, market}], fetcher);
  useEffect(()=>{
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center">
        ...
        <select className="border-primary-light/50 border-2 rounded-lg px-3 h-12 font-semibold w-32" value={market} onChange={e=>setMarket(e.target.value)} name="market" id="">
          <option value="">Markets</option>
          <option className="bg-primary-light" value="3 Way">3 Way</option>
          <option className="bg-primary-light" value="Back & Lay(3 Way)">Back & Lay(3 Way)</option>
        </select>
      </div>
      {
        isLoading?
